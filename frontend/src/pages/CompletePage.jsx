...
import { CheckCircle2, Filter, FilterIcon } from 'lucide-react'
...
const CompletePage = () => {
  ...
  const [sortBy,setShortBy] = useState('newest');
      ...
      <div className={CT_CLASSES.header}>
        <div className={CT_CLASSES.titleWrapper}>
          <h1 className={CT_CLASSES.title}>
            ...
            <span className='turncate'>Completed Tasks</span>
          </h1>
          ...
          <div className={CT_CLASSES.sortBox}>
            ...
            <select value={sortBy} onChange={e => setShortBy(e.target.value)}
              ...
              <div className={CT_CLASSES.btnGroup}>
                {SORT_OPTIONS.map(opt =>(
                  <button key={opt.id} onClick={() =>setShortBy(opt.id)}
                  className={[
                    CT_CLASSES.btnBase,
        ...
        {sortedCompletedTasks.length === 0 ? (
          <div className={CT_CLASSES.emptyState}>
            ...
            <p className={CT_CLASSES.emptyText}>
              Complete ome tasks and they'll appear here
            </p>
          </div>
        ):(
