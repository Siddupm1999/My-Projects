      ...
      if (data.success) {
        setCurrentUser((prev) => ({
          ...
          avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(
            profile.name)}&background=random`,
        }));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-15 h-15 rounded-full bg-gradient-to-br from-fuchsia-500 to-purple-600 flex items-center justify-center text-white text-2xl font-bold shadow-md">
            ...
            <form onSubmit={saveProfile} className="space-y-4">
              {personalFields.map(({ name, type, placeholder, icon: Icon }) => (
                <div key={name} className={INPUT_WRAPPER}>
                  <Icon className="text-purple-500 w-5 h-5 mr-5" />
                  <input
                    type={type}
          ...
          <section className={SECTION_WRAPPER}>
            ...
            <form onSubmit={changePassword} className="space-y-4">
              {securityFields.map(({name,placeholder})=>(
                <div key={name} className={INPUT_WRAPPER}>
                  <Lock className="text-purple-500 w-5 h-5 mr-5" />
                  <input
                    type="password"
                    placeholder={placeholder}
